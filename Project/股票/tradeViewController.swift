...
private extension tradeViewController {
    ...
    func updateNavBar(_ isEditing: Bool = false) {
        if isEditing {
            let doneButton = UIBarButtonItem(title: "完成", style: .done, target: self, action: #selector(editStocks))
            ...
            let editButton = UIBarButtonItem(title: "編輯", style: .plain, target: self, action: #selector(editStocks))
...
extension tradeViewController: SelectStock {
