...
private extension Finnhub.Profile {
    ...
    var mainSection: DetailSection? {
        ...
        if let value = marketCapDisplay {
            let marketCapItem = DetailItem(subtitle: "市值", title: value)
            items.append(marketCapItem)
        }
        
        if finnhubIndustry == "Automobiles"{
            industry = "汽車製造"
        }else if finnhubIndustry == "Technology"{
            industry = "科技"
        }else if finnhubIndustry == "Media"{
            industry = "多媒體"
        }else if finnhubIndustry == "Road&Rail"{
            industry = "運輸"
        }
        
        let section = DetailSection(header: industry, items: items)

        return section
    }
    
    var graphsection: DetailSection? {
        
        var items: [DetailItem] = []
        let nameItem = DetailItem(title: "歷史價格表與K線")
        items.append(nameItem)
        let section = DetailSection(header: "圖表📈", items: items)
        return section
    }
    ...
    var exchangeSection: DetailSection? {
        ...
        if let value = ipoTimeAgo {
            let ipoItem = DetailItem(subtitle: "\(ipoDisplay ?? "") 首次公開募資", title: value)
            items.append(ipoItem)
        }

        let string: [String] = ["已發行股票"]
    ...
    var ipoTimeAgo: String? {
        ...
        rdf.locale = Locale(identifier: "zh-Hant")
...
private extension DetailSection {
    ...
    static func section(_ news: [Finnhub.News]?) -> DetailSection? {
        ...
        let section = DetailSection(header: "新聞", items: items)
